...
public class EnumerableOneOfExtensions
{
    ...
    {
        // Arrange
        var source = new List<OneOf<int, string>>
        {
            1,
            "3"
        };
    ...
    public async Task WhereT0_OnT0T1OneOfEnumerable_CallsFallbackOnT1Items()
    {
        ...
        // Act
        _ = source.OfT0(s => fallbackCalls.Add(s)).ToArray();

        // Assert
        await Assert.That(fallbackCalls).IsEquivalentTo(["3"]);
    }
}