...
public class TUnitValueExtensions
{
    ...
    {
        await Assert.That(Success).SucceededAndValue(x => x.IsEqualTo(11));
    }
    ...
    public async Task SucceedsAndValue_WithFailure_ThrowsAssertionException()
    {
        await Assert.ThrowsAsync<AssertionException>(async () =>
            await Assert.That(Failure).SucceededAndValue(x => x.IsEqualTo(42)));
    }
    ...
    public async Task FailsAndProblemCollection_WithFailure_UnwrapsProblem()
    {
        await Assert
            .That(Failure)
            .FailedAndProblemCollection(x => x.Satisfies(y => y?.Count() == 1));
    }
    ...
    {
        await Assert.ThrowsAsync<AssertionException>(async () =>
            await Assert.That(Success).FailedAndProblemCollection(x => x.Satisfies(y => y != null)));
    }
}