...
public class CopyPropertiesGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var toGenerate = context
            ...
            .ForAttributeWithMetadataName(
                ...
                transform: static (ctx, _) => GetClassOrStructToGenerate(ctx));

        
        context.RegisterSourceOutput(toGenerate, static (spc, source) =>
        {
    ...
    private static (GeneratedTypeModel? Model, string Log, bool Success) GetClassOrStructToGenerate(GeneratorAttributeSyntaxContext context)
    {
        ...
        sb.Append($"Found {allAttributes.Length} attributes with names: {string.Join(", ", allAttributes.Select(a => a.Name.ToString()))}\t");

        var copyPropertyAttributes = allAttributes.Where(a => a.Name.ToString() == CopyPropertiesAttributeHelper.AttributeName)
            .ToArray();
        ...
        var model = GeneratedTypeExtractionHelper.ExtractGeneratedType(sourceSymbols, destinationSymbol, destinationDeclaration);
